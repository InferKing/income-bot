...
## Retention рыночных данных

Scheduler каждый час удаляет `orderbook_snapshots` старше семи дней. Удаление выполняется
короткими транзакциями по 10 000 строк, не более 1 000 000 строк за один запуск. Ошибка очистки
не останавливает сбор рынка: задача повторяется через 15 минут и отражается в состоянии scheduler.

`feature_vectors` retention-политикой не затрагиваются и хранятся бессрочно. Поэтому уже
рассчитанные признаки остаются доступны обучению, но повторное построение признаков из сырого
стакана за период старше семи дней невозможно.

PostgreSQL повторно использует страницы, освобождённые обычным `DELETE`, но размер файлов базы
не обязан уменьшаться сразу. `VACUUM FULL` нельзя запускать автоматически: операция требует
дополнительного места и блокирует таблицу, поэтому выполняется только в согласованное окно.
