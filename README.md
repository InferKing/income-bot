...
комиссий и дополнительного запаса. Рабочий порог уверенности выбирается на validation-части
обучающих данных по доходности после расчётных комиссий; test-часть используется только для
независимой проверки кандидата. Для допуска нужно не менее 30 торговых меток, не менее
`max(20, 40% торговых меток)` сделок и минимум три прибыльных временных окна из четырёх. Пока
champion отсутствует, проверка выполняется раз в час, но новый кандидат создаётся только после
накопления минимум 12 новых размеченных точек.
